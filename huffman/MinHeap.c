                      ...
                      void (*setWeight)(void *e, int weight), int (*getWeight)(void *e))
{
    ...
    minheap->arr = (void**) calloc (capacity, sizeof(char *));
...
MinHeap* insert_minheap(MinHeap* heap, void *element)
{
    ...
    if (heap->size == heap->capacity) {
        fprintf(stderr, "Cannot insert element. Heap is already full!\n");
        return heap;
    }
...
MinHeap* heapify(MinHeap* heap, int index)
{
    ...
    if (left < heap->size && heap->compare(heap->arr[index], heap->arr[left]) > 0)
        smallest = left; 
    
    // Similarly for the right, but we are updating the smallest element
    // so that it will definitely give the least element of the subtree
    if (right < heap->size && heap->compare(heap->arr[smallest], heap->arr[right]) > 0) {
        smallest = right;
    }
...
void* delete_element(MinHeap* heap, int index)
{
    ...
    void *ret = heap->arr[index];
    
    heap->setWeight(heap->arr[index], heap->getWeight(heap->arr[0]) - 1);
    
    // Now keep swapping, until we update the tree
    int curr = index;
    if (curr > 0 && heap->compare(heap->arr[getParentIdx(curr)], heap->arr[curr]) > 0) {
        void *temp = heap->arr[getParentIdx(curr)];
        heap->arr[getParentIdx(curr)] = heap->arr[curr];
        heap->arr[curr] = temp;
        curr = getParentIdx(curr);
